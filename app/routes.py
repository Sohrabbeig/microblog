...
from app.forms import EditProfileForm, EmptyForm, LoginForm, RegisterationForm, \
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        send_email("New user", app.config["MAIL_USERNAME"], app.config["ADMINS"],\
            "A new user with username: {} and email: {} registered!".format(form.username.data, form.email.data))
...
def explore():
    ...
    return render_template('index.html', tilte="Explore", posts=posts, \
        next_url=next_url, prev_url=prev_url)
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        if user is not None:
            ...
            flash("A reset-password email is sent to your email address.")
            return redirect(url_for('login'))
